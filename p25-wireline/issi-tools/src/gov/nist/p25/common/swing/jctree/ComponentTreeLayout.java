...
public class ComponentTreeLayout extends AbstractLayout
   implements ComponentTreeConstants
{
   ...
   public ComponentTreeLayout(TreeModelListener listener)
   {
      ...
      this(listener, EAST, CENTER, SQUARE, 20, 20);
   }

   public ComponentTreeLayout(
      TreeModelListener listener, int hgap, int vgap)
   {
      //this(listener, SOUTH, CENTER, SQUARE, hgap, vgap);
      this(listener, EAST, CENTER, SQUARE, hgap, vgap);
   }
      ...
      int direction, int alignment, int linetype)
   {
      ...
      this(listener, direction, alignment, linetype, 20, 20);
   }
   ...
   private Dimension getMinimumSize(ComponentTreeNode node)
   {
      ...
      if (direction == NORTH || direction == SOUTH)
      {
         ...
         minimumSize = new Dimension(
            Math.max(minimumSize.width, width - vgap),
...
//      int horzInsets = insets.left + insets.right;
   ...
   public void layout(ComponentTreeNode node, int x, int y)
   {
      ...
      if (direction == EAST || direction == WEST)
      {
         ...
         {
            node.getComponent().setBounds(x - size.width, pos,
      ...
      if (direction == NORTH || direction == SOUTH)
      {
         ...
         {
            node.getComponent().setBounds(pos, y - size.height,
                                          size.width, size.height);
            y -= Math.max(size.height, right.height) + vgap;
         }
   ...
   private Dimension getLayoutSize(ComponentTreeNode node)
   {
      ...
      if (direction == EAST || direction == WEST)
      {
         ...
         layoutSize = new Dimension(width,
         Math.max(layoutSize.height, height - vgap));
      }
      if (direction == NORTH || direction == SOUTH)
      {
//EHC
//showln("getLayoutSize(): N|S vgap="+vgap+" hgap="+hgap);
         int width = 0;
         int height = 0;
         if (children > 0)
         {
         ...
         layoutSize = new Dimension(
            Math.max(layoutSize.width, width - hgap),
            height);
      }
      return layoutSize;
   }
