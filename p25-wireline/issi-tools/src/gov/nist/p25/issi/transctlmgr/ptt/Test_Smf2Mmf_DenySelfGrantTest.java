 ...
 */
public class Test_Smf2Mmf_DenySelfGrantTest extends TestCase
   ...
   public void test() {
      ...
      System.out.println("=============================================\n" + 
            "File: Test_SmfTx_WaitUnstoppableTrigger\n" +
            "Description: Tests SMF Tx unstoppable TX trigger from" +
            "wait state.\n"+ 
            "Reference: Section 7.2.1\n\n" +
            
            "SMF -> PTT REQUEST -> MMF \n" + 
            "SMF <- PTT TRANSMIT WAIT <- MMF\n" +
            "SMF WAIT\n" +
            "SMF UNSTOPPABLE TRIGGER\n" +
            "SMF -> PTT PROGRESS -> MMF (x3)\n" + 
      ...
      try {
         /** Note that the sender will pack three packets at at time in a progress packet */
         List<IMBEVoiceBlock> imbeVoiceBlocks = IMBEVoiceDataGenerator
               .getIMBEVoiceData("voice/imbe-hex-test.txt", NUM_AUDIO_PACKETS * 3);
         smfSession.smfTransmitter.setDenyUnstoppableTrigger();
   ...
   public void tearDown() throws Exception {
      ...
      assertTrue("Unxpected grant",gotGrant == false);
      assertTrue("Expected to get a DENY", gotDeny == true);
      assertTrue("Unepxected Wait",gotWait == false);

      /* Check MmfRx assertions */
      assertTrue(gotRequest == true);
      assertTrue(gotRequestWithVoice == false);
      assertTrue(gotAudio == true);
      assertTrue("Expected " + NUM_AUDIO_PACKETS + " Got " + gotAudioPackets ,gotAudioPackets == NUM_AUDIO_PACKETS);
