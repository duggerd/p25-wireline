 ...
 */
public class Test_Smf2Mmf_WaitThenDenyTest extends TestCase
   ...
   public void test() {
      ...
      System.out.println("=============================================\n"
            + "File: Test_SmfTx_MmfWaitThenDeny\n"
            + "Description: Tests denial of SMF request by MMF after "
            + "wait period.\n" + "Reference: Sections 7.2.1 and 7.2.2\n\n" +

            "SMF -> PTT TRANSMIT REQUEST -> MMF\n" + "MMF ARBITRATE\n"
            + "SMF <- PTT TRANSMIT WAIT <- MMF\n" + "SMF WAIT\n"
            + "SMF <- PTT TRANSMIT DENY <- MMF\n"
      ...
      try {
         ...
         mmfSession.getMmfReceiver().setArbitrateWaitThenDeny(5000);
